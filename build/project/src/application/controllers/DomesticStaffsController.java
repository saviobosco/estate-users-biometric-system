...
import application.Retrievals.DomesticStaffsRetrievalService;
...
public class DomesticStaffsController implements Initializable {
	...
	@FXML
	private TableView<DomesticStaff> domesticStaffsTableView;
	
	@FXML
	private TableColumn<DomesticStaff, String> colFullName;
	
	@FXML
	private TableColumn<DomesticStaff, String> colPhoneNumber;
	
	@FXML
	private TableColumn<DomesticStaff, String> colEmailAddress;
	
	@FXML
	private TableColumn<DomesticStaff, String> colPropertyUnitName;
	
	@FXML
	private TableColumn<DomesticStaff, String> colDesignation;
	
	@FXML
	private TableColumn<DomesticStaff, String> colGender;
	...
	public void handleMouseAction(MouseEvent event) {
		if (event.getSource() == domesticStaffsTableView) {
			
		    DomesticStaff selectedRecord = domesticStaffsTableView.getSelectionModel().getSelectedItem();
		    ...
		    if (selectedRecord != null) {
				...
				mi1.setOnAction((ActionEvent actionEvent) -> {
				    ...
				    fxmlLoader.setLocation(getClass().getResource("/application/fxml/EditDomesticStaff.fxml"));
				    
				    // open the resident edit.
					try {
						Parent root = fxmlLoader.load();
					    EditDomesticStaffController controller = fxmlLoader.getController();
						...
						primaryStage.setTitle("Edit Domestic Staff");
				...
				domesticStaffsTableView.setContextMenu(contextMenu);
		    }
		}
	}
	...
	public ObservableList<DomesticStaff> getEstateStaffsList() {
		
		ObservableList<DomesticStaff> estateStaffsList = FXCollections.observableArrayList();
		
		String url1 = "http://griffontech-estate-manager.herokuapp.com/api/v1/domestic-staffs";
		
		Service<ObservableList<DomesticStaff>> service = new DomesticStaffsRetrievalService(url1, this.webServiceAccessToken);
	...
	{
		ObservableList<DomesticStaff> list = getEstateStaffsList();
		
		colFullName.setCellValueFactory(new PropertyValueFactory<DomesticStaff, String>("fullName"));
		colPhoneNumber.setCellValueFactory(new PropertyValueFactory<DomesticStaff, String>("phoneNumber"));
		colEmailAddress.setCellValueFactory(new PropertyValueFactory<DomesticStaff, String>("emailAddress"));
		colPropertyUnitName.setCellValueFactory(new PropertyValueFactory<DomesticStaff, String>("homeAddress"));
		colDesignation.setCellValueFactory(new PropertyValueFactory<DomesticStaff, String>("designation"));
		colGender.setCellValueFactory(new PropertyValueFactory<DomesticStaff, String>("gender"));

		domesticStaffsTableView.setItems(list);
	}
