...
public class EditEstateStaffController implements Initializable {
	...
	private EstateStaff estateStaff;
	...
	private EstateStaffsController parentController ;
	...
	public void initData(EstateStaff estateStaff, String token, EstateStaffsController parentController) {
		this.webServiceAccessToken = token;
		this.estateStaff = estateStaff;
		this.parentController = parentController;
		
		tfFirstName.setText(estateStaff.getFirstName());
		tfMiddleName.setText(estateStaff.getMiddleName());
		tfLastName.setText(estateStaff.getLastName());
		tfPhoneNumber.setText(estateStaff.getPhoneNumber());
		tfEmailAddress.setText(estateStaff.getEmailAddress());
		tfDesignation.setText(estateStaff.getDesignation());
		tfHomeAddress.setText(estateStaff.getHomeAddress());
		
		if (rbGenderMale.getText().equalsIgnoreCase(estateStaff.getGender())) {
			rbGenderMale.setSelected(true);
		}
		
		if (rbGenderFemale.getText().equalsIgnoreCase(estateStaff.getGender())) {
			rbGenderFemale.setSelected(true);
		}
		
		leftThumbCheckbox.setSelected((estateStaff.getLeftThumbFingerPrintTemplate() != null &&  estateStaff.getLeftThumbFingerPrintTemplate().length > 0) ? true : false);
		leftIndexCheckbox.setSelected((estateStaff.getLeftIndexFingerPrintTemplate() != null && estateStaff.getLeftIndexFingerPrintTemplate().length > 0) ? true : false);
		rightThumbCheckbox.setSelected((estateStaff.getRightThumbFingerPrintTemplate() != null && estateStaff.getRightThumbFingerPrintTemplate().length > 0) ? true : false);
		rightIndexCheckbox.setSelected((estateStaff.getRightIndexFingerPrintTemplate() != null && estateStaff.getRightIndexFingerPrintTemplate().length > 0) ? true : false);
		
		// Reassigning the previous fingerprint details.
		left_thumb = estateStaff.getLeftThumbFingerPrintTemplate();
		left_index = estateStaff.getLeftIndexFingerPrintTemplate();
		right_thumb = estateStaff.getRightThumbFingerPrintTemplate();
		right_index = estateStaff.getRightIndexFingerPrintTemplate();
	}
	...
	public void handleButtonAction(ActionEvent event) {
		...
		if (event.getSource() == btnSaveRecord) {
				...
				if (firstName.trim().length() > 1 && phoneNumber.trim().length() > 1 && emailAddress.trim().length() > 1
					...
					HttpPut httpPut = new HttpPut("http://griffontech-estate-manager.herokuapp.com/api/v1/estate-staffs/" + estateStaff.getId());
